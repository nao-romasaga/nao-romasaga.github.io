...
// 特殊計測、スタイル毎に使用技を固定する普段は使わない
let ACTION_PATTERN = {
    //"ID3a669":["豊穣の息吹","瞬風刃","豊穣の息吹","ファータイルガスト","豊穣の息吹","シェイドボール","豊穣の息吹","ファータイルガスト","豊穣の息吹","シェイドボール","豊穣の息吹","ファータイルガスト","豊穣の息吹","シェイドボール","豊穣の息吹","ファータイルガスト"],
    "ID3a669":["豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃","豊穣の息吹","瞬風刃",],
    //"ID3978b":["勇猛の鬨","通常攻撃","猛烈斬り+","勇猛の鬨","猛烈斬り+","猛烈斬り+","勇猛の鬨","通常攻撃","猛烈斬り+","猛烈斬り+","勇猛の鬨","猛烈斬り+","猛烈斬り+",],
    "ID3978b":["猛烈斬り+","勇猛の鬨","猛烈斬り+","猛烈斬り+","通常攻撃","勇猛の鬨","猛烈斬り+","猛烈斬り+","通常攻撃","勇猛の鬨"],
    //"ID37f5b":["剣豪の構え","炎獄交叉","剣豪の構え","炎獄交叉","炎獄交叉"],
    //"ID3801c":["","タイダルスピア","タイダルスピア","タイダルスピア","タイダルスピア","タイダルスピア","タイダルスピア","タイダルスピア"],
}

//$(".jutsuIryoku").slideUp();

let url = "https://romasagatool.com/api/auto_ranking.php?limit=150&table=&dev=dev&enemy=1&dup=true&bp=10&turn=5&multi=0,2&round=T&wp=STR101502100,INT101501100,STR102502300,INT102502000,STR102501900,INT102501100,INT103500600,STR103500400,INT104501200,INT104500600,STR104500500,AGI105501100,INT105501000,DEX106500700,DEX107501200,INT107500800,INT107500500,INT108501100,STR108500900,INT108500600,INT109500700,DEX109500500,INT109500400,INT160500100,INT155500500,INT163500100,INT161500100,INT156500500,INT154500400,INT152500100,INT151500400,INT165500100,INT158500100,INT157500100";
//$.getJSON(`${url}&callback=?`);
function callback(result){
    let topRanker = {};
    for(data of result) {
        if(data['enemyclass'] == 0 ) {
            continue;
        }
        if(typeof topRanker[data['sid']] == "undefined") {
            topRanker[data['sid']] = data;
        }
        if(Object.keys(topRanker).length > 10){
            break;
        }
    }

    $("#SET_TURN").val(10);
    $(".STR-WEAPON").find(".WEAPON_CHOISE").click();
    ...
    $("#enemy_vit").val(600);
    $("#enemy_mnd").val(600);    
    for(styleId in topRanker){
        NOW_STYLE = STYLE_MASTER[styleId];
        let charId = NOW_STYLE['cId'];
        ...
        clickStyle(styleId);
        const skilllist = [];
        for(skills of topRanker[styleId]['kakusei'].split("|")) {
            [skillId, kakusei] = skills.split(":");
            console.log(skills, skillId, kakusei);
            skillInfo = Object.assign({},SKILL_MASTER[skillId]);
            skillInfo['UseBp'] = Number(skillInfo['ConsumeBp']) - Number(kakusei);
            skilllist.push(skillInfo);
            console.log(skillInfo['Name']);
        }
        displayResult(skilllist);
        console.log(NOW_STYLE['Name'], topRanker[styleId]['damage']*5);
    }
...
let withMusume = false; // 海の宴＝偶数ターンBP回復
let withSaruin = false;
let withSubie = false;
let withRazem = false;
let withMaria = false;
let withPatric = false;
let withShijin = false;
...
const STYLE_SWITCH = [
    {"id":"ID357df", "text":"陰属性攻撃BP+1<br>陰+50%"}, // サルーイン
    // {"id":"IDadf9a", "text":"毎T腕器知+15%<br>ﾀﾞﾒｰｼﾞ+20%"}, // ラゼム
    // {"id":"ID36832", "text":"奇T全ﾊﾞﾌ25%"}, // マリア
    {"id":"IDc5ce0", "text":"毎TBP+1"}, // 詩人
    // {"id":"ID3801f", "text":"奇T:冷突+35% 全ﾊﾞﾌ20% ﾀﾞﾒｰｼﾞ+15%<br>偶T:冷突+15% 全ﾃﾞﾊﾞﾌ10%"}, // スービエ
    // {"id":"ID33fa5", "text":"毎T器知+15%<br>ﾀﾞﾒｰｼﾞ+10%"}, // パトリック
];
...
// ターン進行とともに追加されるアビリティ
let ADD_TURN_AB_TSUIGEKI = {
    "ID29d938ef":{
        1:[{ "targetSkill":"any" ,"PER":50, "addSkill":["ID69185ef"] },], // サクぎり
        2:[{ "targetSkill":"any" ,"PER":37, "addSkill":["ID69185f0"] },], // ムーンスクレイパー
        3:[{ "targetSkill":"any" ,"PER":37, "addSkill":["ID69185f1"] },], // 名月斬
        4:[{ "targetSkill":"any" ,"PER":25, "addSkill":["ID69185f2"] },], // ムーンヘイズ
    },
    "ID29d93ab1":{ // 強靭な心
        5:[
            { "targetSkill":"single" ,"PER":100, "addSkill":["ID68fffdc"] },
            { "targetSkill":"all" ,"PER":100, "addSkill":["ID68fffda"] },
        ],
        10:[
            { "targetSkill":"single" ,"PER":100, "addSkill":["ID68fffdc"] },
            { "targetSkill":"all" ,"PER":100, "addSkill":["ID68fffda"] },
...
const SENKOU_ADD_ATTACK = ["バイオレットナイト","サンフレアバースト","くるくるサンダー","翼弓舞鴉","タイダルスピア"];
let HOJYO_TSUIGEKI = {
    "ID69c3d8b" : [// 翼弓舞鴉
        {"targetSkill":"any" ,"PER": 50, "addSkill":["ID69c3d8d"]},
    ],      
    "ID774e143" : [// サンフレア
        {"targetSkill":"any" ,"PER": 100, "addSkill":["ID774e149"], "limit": {"when":"ターン", "time":3}}, // 直接追加してるから1回少ない
    ],      
    "ID77c82b1" : [ // バイオレットナイト
        {"targetSkill":"any" ,"PER": 100, "addSkill":["ID77c82b2"], "limit": {"when":"ターン", "time":3}}, // 直接追加してるから1回少ない
    ],     
    "ID68fffd5": [ // くるくるサンダー
        { "targetSkill":"any" ,"PER":100, "addSkill":["ID68fffd7","ID68fffd7"] , "limit": {"when":"ターン", "time":1}},
    ],

    "ID69619d7":[{ "targetSkill":"single" ,"PER":100, "addSkill":["ID69619d9"] }], // ドラゴンフォース
    "ID697a012":[{ "targetSkill":"single" ,"PER":100, "addSkill":["ID697a013"] }], // アイスチェイサー リボルヴ
    "ID697a036":[{ "targetSkill":"single" ,"PER":100, "addSkill":["ID697a038"] }], // アタックチェイサー
    "ID697a00f":[{ "targetSkill":"single" ,"PER":100, "addSkill":["ID697a011"] }], // ウインドチェイサー
    "ID697a04f":[{ "targetSkill":"single" ,"PER":100, "addSkill":["ID697a052"] }], // フローズンチェイサー
    "ID697a064":[{ "targetSkill":"single" ,"PER":100, "addSkill":["ID697a065"] }], // アンチマテリアルショット
    // 技のIDはdata_SKILLMASTER_autoから確認
    "ID697a030":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID697a032"] ,"limit": {"when":"バトル中", "time":3}}], // ラビット射出
    "ID697a020":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID697a021"] ,"limit": {"when":"バトル中", "time":3} }], // クロービット強襲
    "ID68fff40":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID68fff41"] ,"limit": {"when":"バトル中", "time":1} }], // 忍法神速プレゼント剣（プレゼント忍法）
    "ID68fff41":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID68fff41"] ,"limit": {"when":"バトル中", "time":1} }], // 忍法神速プレゼント剣（プレゼント忍法）
    "ID77974a9":[{ "targetSkill":"overdrive攻撃" ,"PER":100, "addSkill":["ID776678a"], 'TURN':3}], // つつじの舞の効果
    "ID6930c5b":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID6930c5e"] ,'TURN':4}], // ガーネットダンス
    "ID7766795":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID7766798"] ,'TURN':4}], // ねんどうチャージ
    "ID697a0eb":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID697a0ec"] ,"limit": {"when":"バトル中", "time":1} } ], // 殲滅システム
    "ID68fffe3":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID68fff67"] ,'TURN':4}], // 諸刃の雄叫び
    "ID6992738":[{ "targetSkill":"any" ,"PER":100, "addSkill":["ID699273b"] }], // ウェイクアップディフェンド
    "ID77c82bd":[ // 闇の開花
        { "targetSkill":"any" ,"PER":100, "addSkill":["ID77c82c2"] ,"exclude" :["ID77c82c0"]}, // 魔の風
        { "targetSkill":"any" ,"PER":50, "addSkill":["ID77c82c1"] ,"exclude" :["ID77c82c0"]}, // ブラックディメンション
    ], 
    "ID69aadcb":[
        { "targetSkill":"overdrive攻撃" ,"PER":100, "addSkill":["ID69aadcd"] ,'TURN':5 ,"limit": {"when":"バトル中", "time":1} }
    ], // 海王の猛攻
}
...
function displayResult(skillList = []) {
    ...
    // 使う技のBP書き換え
    skillList = rewriteBP(skillList);
    // 武器威力、耐性などを追加
    for (let skillInfo of skillList) {    
        setSkillCalcBase(skillInfo);
    }
    // 開始前にアビリティのバフデバフをチェック
    setStyleAbility();
    
    if(DEBUG) {
        console.log("skillList",JSON.parse(JSON.stringify(skillList)));
        console.log("BUFF_ABILITY",JSON.parse(JSON.stringify(BUFF_ABILITY)));
        console.log("DAMAGE_ABILITY",JSON.parse(JSON.stringify(DAMAGE_ABILITY)));
        console.log("ADD_ABILITY",JSON.parse(JSON.stringify(ADD_ABILITY)));        
        console.log("OD_ABILITY",JSON.parse(JSON.stringify(OD_ABILITY)));
        console.log("TSUIGEKI_ABILITY",JSON.parse(JSON.stringify(TSUIGEKI_ABILITY)));    
        console.log("COUNTER_ABILITY",JSON.parse(JSON.stringify(COUNTER_ABILITY)));        
        console.log("======== シミュレーションスタート ========");
    }
...
for(const jp in MASTER_LV_KEY) {
    MY_MASTER_LV[MASTER_LV_KEY[jp]] = WEAPON_MAP[jp]['master'];
}
let LOGIN_FLG = false;
...
$(document).ready(function ($) {
    for(data of STYLE_SWITCH) {
        let styleId = data['id'];
        let styleInfo = STYLE_MASTER[styleId];
        let styleImg = getImgPath(`style_icon_bg/${styleId}.png`);
        let tmpl1 = $("#WITH_STYLE_TEMPLATE1").clone().removeAttr("id").removeClass("d-none");
        tmpl1.find(".ONOFF_STYLE").attr('data-value', styleInfo['Name']);
        tmpl1.find(".NAME").html(styleInfo['Name']);
        tmpl1.find(".style").attr("style",`background: url(${styleImg}) no-repeat; background-size:contain;`);
        let tmpl2 = $("#WITH_STYLE_TEMPLATE2").clone().removeAttr("id").removeClass("d-none");
        tmpl2.find(".TEXT").html(data['text']);
        $("#WITH_STYLE_AREA").append(tmpl1);
        $("#WITH_STYLE_AREA").append(tmpl2);
    }
    ...
    if (location.search !== "") {
        ...
        if (STYLE_MASTER[styleId] !== undefined) {
            let charId = STYLE_MASTER[styleId]['cId'];
            //Title変更
    ...
    $(".ONOFF_STYLE").click(function(){
        const styleName = $(this).attr("data-value");
        const v = $(this).hasClass("icon_btn_negative");
        if(styleName == "サルーイン") {
            withSaruin = v;
        } else if(styleName == "スービエ") {
            withSubie = v;
        } else if(styleName == "ラゼム") {
            withRazem = v;
        } else if(styleName == "マリア") {
            withMaria = v;
        } else if(styleName == "パトリック") {
            withPatric = v;        
        } else if(styleName == "詩人") {
            withShijin = v;
        }
        toggleButton($(this), v);
    });
    ...
    $(".culcStart").click(function () {
        ...
        isLoop = ($(this).attr("data-type") === "loop");
        displayResult();
 ...
 * @returns {undefined}
 */
function clickChar(charId, styleId = "") {
    $("#styleLabel").show();
    NOW_CHAR = CHAR_MASTER[charId];
    // 1件目はデフォルトで出しちゃう
    ...
    $("#NOW_MAX_ML").html(WEAPON_MAP[NOW_STYLE['WeaponType']]['master']);
    $("#SET_MLV").html(WEAPON_MAP[NOW_STYLE['WeaponType']]['master']);
    if(MY_MASTER_LV[MASTER_LV_KEY[NOW_STYLE['WeaponType']]]) {
        //$("#SET_MLV").val(MY_MASTER_LV[MASTER_LV_KEY[NOW_STYLE['WeaponType']]]);
    }
...
function clickStyle(styleId) {
    history.replaceState('','',`auto.html?s=${styleId}`);
...
function addSkillArea(skillInfo, target) {
    skillInfo['SkillIryoku'] = ((skillInfo['SkillIryoku'] == "" || skillInfo['SkillIryoku'] == 0) && skillInfo['PowerGrade'] != '-') 
    ? IRYOKU_MAP[skillInfo['PowerGrade']] : skillInfo['SkillIryoku'];

    let $row = $("#SKILL_TR_TEMPLATE").clone().removeAttr("id").removeClass("d-none").addClass("SKILL_TR");
    $row.attr("data-id", skillInfo['Id']);
    $row.find(".skill").attr("data-id", skillInfo['Id']);
    $row.find(".SKILL_NAME").html(skillInfo['Name']);

    for (let kakusei = 1; kakusei <= skillInfo['Kakusei']; kakusei++) {
        $row.find(".KAKUSEI"+kakusei)
            .removeClass("d-none")
            .addClass('kakusei' + skillInfo['Id'])
            .attr("data-id", skillInfo['Id']).attr("data-kakusei", kakusei);
    }
    skillInfo['isNotUseAuto'] = isNotUseAuto(skillInfo);
    // 使わない場合はOFFにする
    $row.attr('data-onoff', 1);
    if(skillInfo['isNotUseAuto']){
        $row.attr('data-onoff', 0);
        $row.find(".ON_OFF").toggleClass("icon_btn_positive").toggleClass("icon_btn_negative");
        $row.find(".ON_OFF > .TOGGLE").toggleClass("fa-toggle-on").toggleClass("fa-toggle-off");
        $row.find(".ON_OFF > .BTN_LABEL").text("OFF");
    }
    if(skillInfo['Rentatsu'] == undefined || skillInfo['Name'].indexOf("+") > -1){
        $row.find(".RENTATSU").addClass("d-none");
    }

    //BattleType , AttackAttributes
    let battleIconPath = getImgPath('icon/' + ICON_LIST[skillInfo['BattleType']] + ".png");
    $row.find(".ICON").append($(`<img src="${battleIconPath}" class="icon_xs">`));

    skillInfo['AttackAttributes'].split(',').forEach(function (value) {
        let attrIconPath = getImgPath('icon/' + ICON_LIST[value] + ".png");
        $row.find(".ICON").append($(`<img src="${attrIconPath}" class="icon_xs">`));
    });
    $row.find(".SKILL_BP").addClass(`nowBP${skillInfo['Id']}`).attr("data-id", skillInfo['Id']).attr("data-bp",skillInfo['ConsumeBp']).html(skillInfo['ConsumeBp']);
    $row.find(".SKILL_IRYOKU").html(`${skillInfo['PowerGrade']}(${skillInfo['SkillIryoku']})`);
    $row.attr("data-keisho", skillInfo['isKeisho']);
...
// ターン毎に変わらないものはここで設定する
// キャラステータス、敵体力は変動するのでここには入れない
function setSkillCalcBase(skillInfo) {
    if(skillInfo['SkillType'] === "術") {
        let idx = Object.values(WEAPON_MASTER).indexOf(skillInfo['BattleType']);
        let jutsuNo = Object.keys(WEAPON_MASTER)[idx];
        weapon = $("#SET_WEAPON_"+jutsuNo).val();
    } else {
        weapon = $("#SET_WEAPON").val();
    }
    if (skillInfo['Name'] === "通常攻撃") {
        skillInfo['BattleType'] = NOW_STYLE['WeaponType'];
        skillInfo['AttackAttributes'] = WEAPON_ATTR[NOW_STYLE['WeaponType']];
    }

    let attr = skillInfo['AttackAttributes']; // 斬/打/突
    let resist = 999;
    if (attr !== undefined) {
        attr.split(",").forEach(function (x) {
            r = $("#taisei_" + RESIST_MAP[x]).val();
            if (resist > Number(r)) {
                resist = Number(r);
            }
        });
    }
    let iryoku = ((skillInfo['SkillIryoku'] == "" || skillInfo['SkillIryoku'] == 0) && skillInfo['PowerGrade'] != '-') 
    ? IRYOKU_MAP[skillInfo['PowerGrade']] : skillInfo['SkillIryoku'];    
    skillInfo['culcData'] = [weapon, iryoku, skillInfo['rank'] ?? 99, resist];
}

let MASTER_LV_ABILITY = 0;
function culcAutoMode(skillList) {
    let maxTurn = $("#SET_TURN").val();
    //maxTurn = (isLoop) ? 100 : maxTurn;
    // culcStyleBonus依存
    ...
    //initBp = (isLoop) ? 3 : initBp; // ループの時はBP0からスタート
    let bp = initBp;
    ...
    let tsuigekiCount = 0;
    if(withShijin && NOW_STYLE['Id'] != "IDc5ce0") {
    ...
    for(i in NOW_STYLE['StyleAbilityIds']){
        ...
        if(typeof ADD_TURN_AB_TSUIGEKI[abId] != "undefined") {
            addTsuigeki = ADD_TURN_AB_TSUIGEKI[abId];
        }
    }
    ...
    for (let turn = 1; turn <= maxTurn; turn++) {
        ...
        if(NOW_WAITING["wait"] > 0){
            ...
            for (let skill of tmpSkillList) {
                ...
                // 追撃発生
                // 攻撃前追撃追加なので、直接追加する
                if(SENKOU_ADD_ATTACK.includes(skill['Name'])){
                    canTsuigeki = setAddAttackFromSkill(skill, canTsuigeki);
                    nextTurnTsuigeki = setAddAttackFromSkill(skill, nextTurnTsuigeki);
                } else {
                    nextTurnTsuigeki = setAddAttackFromSkill(skill, nextTurnTsuigeki);
                }
                ...
                if(typeof skill['isTamewaza'] !== "undefined") {
                    ...
                    for(addSkillId of addSkill) {
                        ...
                        let per = (typeof SKILL_TSUIGEKI[addSkillId] != "undefined") ? SKILL_TSUIGEKI[addSkillId][0]["PER"] : 100;
                ...
                let skillIds = getTsuigekiSkill(skill, turn);

                function getTsuigekiSkill(skill, turn, isHPMax, isOverDrive) {
                    if(skill['Type'] != "攻撃") {
                        return [];
                    }
                    // 追撃チェック
...
function calcDamage(skillInfo, isOverDrive) {
    ...
    let damagePoint = (iryoku > 0) 
        ...
        : 0;
    SKILL_USE_COUNT[skill['Name']] = SKILL_USE_COUNT[skill['Name']] ?? 0;
    SKILL_USE_COUNT[skill['Name']]++;
    let record =  {
        'damage': damagePoint,
        'skill': skillInfo,
        'count':1,
...
function setAfterSkillBuffDeBuff(skillInfo){
    ...
    // なんで補助でスキップしてるか不明
    if(typeof skillInfo['Mod'] === "undefined"
    && typeof HOJYO_TSUIGEKI[skillInfo['Id']] == "undefined") {
        return;
    }
...
function setAddAttackFromSkill(skillInfo, canTsuigeki){  
    ...
    if(typeof HOJYO_TSUIGEKI[skillInfo['Id']] != "undefined") {
        for(tsuigekiInfo of HOJYO_TSUIGEKI[skillInfo['Id']]){
            if(typeof canTsuigeki[tsuigekiInfo["targetSkill"]] == "undefined") {
                canTsuigeki[tsuigekiInfo["targetSkill"]] = [];
            }
            tsuigeki = JSON.parse(JSON.stringify(tsuigekiInfo));
            tsuigeki['TURN'] = (!!tsuigeki['TURN']) ? tsuigeki['TURN'] : 99;
            canTsuigeki[tsuigekiInfo["targetSkill"]].push(tsuigeki);
            // 殲滅システムだけ特殊
            if(skillInfo['Id'] == "ID697a0eb"){
                canTsuigeki[tsuigekiInfo["targetSkill"]].push(tsuigeki);
                canTsuigeki[tsuigekiInfo["targetSkill"]].push(tsuigeki);
            }
        }
...
function rewriteBP(skillList) {
    ...
    if ($("table#skillArea > tfoot .SKILL_TR").length == 0) {
    ...
    // 継承技の上書き。継承技は何よりも優先される
    // skill4が補助じゃない＋無制限で、BP3と同じ場合
    // Skill3が補助で優先するなら潰さない
    if (skill4['AutoUseBp'] === skill3['AutoUseBp'] && _notUse(skill4) && !_isHojyoAndPrimary(skill3)) {
        skill3['AutoUseBp'] = 99; // 継承が3を上書き
    }
    if (skill4['AutoUseBp'] === skill2['AutoUseBp'] && _notUse(skill4) && !_isHojyoAndPrimary(skill2)) {
        skill2['AutoUseBp'] = 99; // 継承が2を上書き
    }
    if (skill4['AutoUseBp'] === skill1['AutoUseBp'] && _notUse(skill4) && !_isHojyoAndPrimary(skill1)) {
        skill1['AutoUseBp'] = 99; // 継承が1を上書き
    }

    if (skill1['AutoUseBp'] === skill2['AutoUseBp'] && _notUse(skill2)) {
        if(skill1['ConsumeBp'] <= skill2['ConsumeBp']) {
            if(!_isHojyoAndPrimary(skill1)){
                skill1['AutoUseBp'] = 99; // 元BPが大きい方を使うので1は消す
            }
        } else if(!_isHojyoAndPrimary(skill2)) {
            skill2['AutoUseBp'] = 99; // 元BPが大きい方を使うので2は消す
        }
    } else if (skill2['AutoUseBp'] === skill3['AutoUseBp'] && _notUse(skill3)) {
        if(skill2['ConsumeBp'] <= skill3['ConsumeBp']) {
            if(!_isHojyoAndPrimary(skill2)){
                skill2['AutoUseBp'] = 99; // 元BPが大きい方を使うので2は消す
            }
        }else if(_notUse(skill3) && !_isHojyoAndPrimary(skill3)) {
            skill3['AutoUseBp'] = 99; // 元BPが大きい方を使うので3は消す
        }
    }
    skill1['UseCount'] = 0;
    skill2['UseCount'] = 0;
    ...
    skill4['UseCount'] = 0;
    skillList.sort((a, b) => {
        if (a.AutoUseBp > b.AutoUseBp) {
            // 基本は使用BP降順
            return -1;
        } else if (a.AutoUseBp === b.AutoUseBp && a.isKeisho >= b.isKeisho && a.ConsumeBp >= b.ConsumeBp && a.SortKey > b.SortKey) {
    ...
    newSkillList = []
    hojoSkillList = []
    for(skillInfo of skillList){
        if(isHojoPrimary && skillInfo['Type'] == "補助") {
            hojoSkillList.push(skillInfo);
        } else if(isLimitPrimary && (typeof skillInfo['LimitCount'] != "undefined" && skillInfo['LimitCount'] > 0)) {
            hojoSkillList.push(skillInfo);
        } else {
            newSkillList.push(skillInfo);
        }
    }
    skillList = [...hojoSkillList, ...newSkillList];
    return skillList;
}
...
function addTsuigekiAbility(abInfo, abCategory) {
    ...
    if(abCategory['main'] != "追撃"
    || AB_TSUIGEKI[abInfo['Id']] != undefined) {
        return;
    }
    let tsuigekiWhen = WHEN_MAP[abCategory['when']];
    if(tsuigekiWhen == undefined) {
        console.log(`追撃判定の ${abCategory['when']} が存在しない`);
        return;
    }
    ...
    arrayPush(TSUIGEKI_ABILITY, tsuigekiWhen, tmp);
}
...
function skipSkillPattern(skill, turn, bp){
    ...
    if(false) {
        if(skill['Id'] == "ID68fffdf" && turn <= 3) { // 2-2-15ループするので八重二振は4T目まで使わない
            //return true;
        }
        if(["ID3601c","IDae062"].includes(NOW_STYLE['Id'])){
            if(["ID6992736","ID68fffdf"].includes(skill['Id']) && (turn -1) % 3 > 0 ) {
                return true;
            }
        }
        if(skill['Id'] == "ID68fffd6") { // ピカピカ花火は1,6,11ターン目にしか使わない
            if((turn -1) % 5 > 0 ) {
                return true;
            }
        }                    
        if(NOW_STYLE['Id'] == "ID94f41"){ // ウルピナの姫騎士の呼吸は1,5ターンにしか使わない
            if(turn != 1 && turn != 6 && skill['Id'] == "ID68fff70") {
                return true;
            }
        }

        if(NOW_STYLE['Id'] == "ID1aede"){
            if((turn -1) % 4 <= 1 ) { // 4の倍数はねんどうチャージしか使わない
                if(skill['Id'] != "ID7766795") {
                    return true;
                }
            }
        }
        if(skill['Id'] == "ID69c3411") {
            if(turn != 1 && turn != 4) {
                return true;
            }
            if(turn == 4 && maxTurn <= 6) {
        ...
        // 回魂の願いは自動復帰のある零姫のみなので特殊処理
        if(skill['Id'] == "ID77c8224"){
            //if(bp < 8 || turn == 1) {
            if(bp <= 10) {
                ConsumeLp = 1;
            } else {
